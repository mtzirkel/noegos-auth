...
export const actions: Actions = {
	default: async ({ request }) => {
		...
		if (!username) {
			return fail(400, { error: 'Username is required', displayName, message });
		}
		...
		if (existing.length > 0) {
			return fail(400, { error: 'Username is already taken', displayName, message });
		}
		...
		`;
		if (pending.length > 0) {
			return fail(400, { error: 'A request for this username is already pending', displayName, message });
		}
